...
parser.add_argument("--q", help="The prefix for cues in comments. \
    Defaults to 'Q'.")
parser.add_argument("--i", help="The prefix for times in comments \
    Defaults to 'I'.")
parser.add_argument("--l", help="The prefix for labels in comments \
    Defaults to 'Label:'.")
parser.add_argument("--n", help="The prefix for labels in comments \
    Defaults to 'Notes:'.")
parser.add_argument("--s", help="The prefix for scenes in comments \
    Defaults to 'Scene:'.")
parser.add_argument("--m", help="The prefix for marks in comments \
    Defaults to 'M'.")
parser.add_argument("--b", help="The prefix for blocks in comments \
    Defaults to 'B'.")
parser.add_argument("--f", help="The prefix for follows in comments \
    Defaults to 'F'.")
parser.add_argument("--x", help="The prefix for executes in comments \
    Defaults to 'X'.")
...
def process_cues(
        annotations, cue_prefix, time_prefix, label_prefix, notes_prefix,
        scene_prefix, mark_prefix, block_prefix, follow_prefix,
        execute_prefix):
    ...
    for i, cue in enumerate(json_cues):
        ...
        for j, comment in enumerate(cue_comment):
            if comment.lower().startswith(cue_prefix.lower()) or \
                    comment.startswith(cue_prefix.lower() + " "):
                cue_number = comment[len(cue_prefix):]

            if comment.lower().startswith(time_prefix.lower()) or \
                    comment.startswith(time_prefix.lower() + " "):
                cue_time = comment[len(time_prefix):]

            if comment.lower().startswith(label_prefix.lower()) or \
                    comment.startswith(label_prefix.lower() + " "):
                cue_label = comment[len(label_prefix):]

            if comment.lower().startswith(notes_prefix.lower()) or \
                    comment.startswith(notes_prefix.lower() + " "):
                cue_notes = comment[len(notes_prefix):]

            if comment.lower().startswith(scene_prefix.lower()) or \
                    comment.startswith(scene_prefix.lower() + " "):
                cue_scene = comment[len(scene_prefix):]

            if comment.lower().startswith(mark_prefix.lower()) or \
                    comment.startswith(mark_prefix.lower() + " "):
                cue_mark = comment[len(mark_prefix):]

            if comment.lower().startswith(block_prefix.lower()) or \
                    comment.startswith(block_prefix.lower() + " "):
                cue_block = comment[len(block_prefix):]

            if comment.lower().startswith(follow_prefix.lower()) or \
                    comment.startswith(follow_prefix.lower() + " "):
                cue_follow = comment[len(follow_prefix):]

            if comment.lower().startswith(execute_prefix.lower()) or \
                    comment.startswith(execute_prefix.lower() + " "):
...
def convert_file(args):
    ...
    md_table = process_cues(
        annotations, cue_prefix, time_prefix, label_prefix, notes_prefix,
        scene_prefix, mark_prefix, block_prefix, follow_prefix, execute_prefix)
